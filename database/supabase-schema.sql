-- ============================================================
-- GoLift — Supabase Schema (PostgreSQL)
...
CREATE TABLE public.profiles (
  ...
  nome TEXT NOT NULL,
  email TEXT NOT NULL,
  tipo INTEGER DEFAULT 0, -- 0=user, 1=admin
  idade INTEGER,
  peso NUMERIC(5,1),
  altura NUMERIC(5,1),
  objetivo TEXT,
...
-- 10. Admin Workouts (workout templates from admin)
...
CREATE INDEX idx_community_members_community ON public.community_members(comunidade_id);
CREATE INDEX idx_community_messages_community ON public.community_messages(comunidade_id);
CREATE INDEX idx_ai_reports_user ON public.ai_reports(user_id);
CREATE INDEX idx_ai_plans_user ON public.ai_plans(user_id);
...
-- Profiles: user vê/edita próprio perfil; admins vêem tudo
...
-- Workouts: user vê/edita próprios treinos
...
-- Workout Sessions: user vê próprias sessões
...
-- Communities: verified are public; members see details
...
-- Community Members: members see members
...
-- Community Messages: members can read and send
